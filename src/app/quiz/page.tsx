...
export default function QuizPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    fetch(GOOGLE_SCRIPT_URL, {
      ...
      body: JSON.stringify({ name, phone: phone.replace(/^\+/, ""), result: p.name, source: "quiz" }),
    }).catch(() => {});
        ...
        <AnimatePresence mode="wait" custom={direction}>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-0">
                <div className="border-t border-[#e8e4df]">
                  <input value={name} onChange={e => setName(e.target.value)}
                    ...
                    className="w-full py-4 text-base font-light text-[#0f172a] bg-transparent outline-none placeholder:text-[#c0bbb5] border-b border-[#e8e4df] transition-colors focus:border-[#0f172a]" />
                </div>
                <div>
          ...
          {stage === "result" && p && (
            <motion.div key="result" variants={slideVariants} initial="enter" animate="center" exit="exit"
              ...
              <div className="mb-12">
                <div className="text-5xl mb-6">{p.figurine}</div>
                <h2 className="serif text-[clamp(2rem,5vw,3.5rem)] leading-[1.1] text-[#0f172a] mb-4">
                  {p.name}
                </h2>
              ...
              <div className="relative mb-10">
                <div className="grid grid-cols-3 gap-6 blur-sm pointer-events-none select-none opacity-60">
                  {[
                    { label: "Strengths", items: p.strengths.slice(0, 2) },
                    { label: "Blind spots", items: p.blindspots.slice(0, 2) },
                    { label: "Next level", items: p.nextLevel.slice(0, 2) },
                  ].map(col => (
                    <div key={col.label}>
                      <p className="text-[10px] tracking-[0.2em] uppercase text-[#9a9490] mb-3">{col.label}</p>
                      {col.items.map(item => (
                        <p key={item} className="text-xs text-[#4a4540] font-light mb-2 leading-snug">{item}</p>
                      ))}
                    </div>
                  ))}
                </div>
                <div className="absolute inset-0 flex items-center justify-center bg-[#FAF8F5]/70 backdrop-blur-[2px]">
                  <div className="text-center px-6">
                    <p className="text-xs tracking-[0.2em] text-[#9a9490] uppercase mb-2">Full report</p>
                    <p className="serif text-lg text-[#0f172a] leading-snug">Our specialist will reach out<br />with your detailed breakdown.</p>
                  </div>
                </div>
              </div>

              {/* Financial age badge */}
              <div className="border border-[#e8e4df] px-6 py-4 flex items-center justify-between mb-10">
                <span className="text-[10px] tracking-[0.2em] text-[#9a9490] uppercase">Financial Age</span>
                <span className="serif italic text-lg text-[#0f172a]">{p.financialAge}</span>
              </div>
